...
const setFontColor = (theme: string, menuLocation: string, themeData: IThemeData) => {
  ...
  if (menuLocation === 'left') {
    ...
    document.documentElement.style.setProperty(
      ...
      `${getTextColor2(calcBrightness(colorData.accentColor), 'var(--color-fontlightback)', 'var(--color-fontdarkback)', 120)}`
    );
  } else if (menuLocation === 'topandleft') {
    document.documentElement.style.setProperty(
      '--color-drawer-font',
      `${getTextColor2(calcBrightness(colorData.backDark1Color), 'var(--color-fontlightback)', 'var(--color-fontdarkback)', 120)}`
    );
    document.documentElement.style.setProperty(
      '--color-drawer-selected-menu-font',
      `${getTextColor2(calcBrightness(colorData.backDark1Color), 'var(--color-fontlightback)', 'var(--color-fontdarkback)', 120)}`
    ...
    document.documentElement.style.setProperty(
      '--color-primary-font',
      `${getTextColor2(calcBrightness(colorData.primaryColor), 'var(--color-fontlightback)', 'var(--color-fontdarkback)', 120)}`
    );
  }

  document.documentElement.style.setProperty(
    '--color-grid-header-font',
    `${getTextColor2(calcBrightness(colorData.gridHeaderColor), 'var(--color-fontlightback)', 'var(--color-fontdarkback)', 120)}`
  );
  document.documentElement.style.setProperty(
    '--color-common-header-font',
    `${getTextColor2(calcBrightness(colorData.commonHeaderColor), 'var(--color-fontlightback)', 'var(--color-fontdarkback)', 120)}`
  );
  document.documentElement.style.setProperty(
...
export const setVerticalSizeDetail = (params: {
  ...
  console.log('height', height, topHeight);
  console.log('part', contentHeight, controlHeight, filterHeight, contentsPadding);

  console.log('--size-content-inner-height-outer-controller', contentsInnerHeightSinglePadding);
  ...
  document.documentElement.style.setProperty('--size-content-height-outer-controller', `${contentHeightOuterController}px`);
  ...
  document.documentElement.style.setProperty('--size-content-inner-height-outer-controller', `${contentsInnerHeightSinglePadding}px`);
  document.documentElement.style.setProperty('--size-content-height-outer-controller-tab', `${contentHeightOuterControllerTab}px`);
  document.documentElement.style.setProperty(
    '--size-content-inner-height-outer-controller-tab',
    `${contentInnerHeightOuterControllerTab}px`
  );
  document.documentElement.style.setProperty('--size-content-inner-height-no-controller', `${contentsInnerHeightNoController}px`);
  document.documentElement.style.setProperty(
    '--size-content-inner-height-no-controller-with-padding',
    `${contentsInnerHeightNoControllerWithPadding}px`
  );
  document.documentElement.style.setProperty('--size-content-inner-height-no-padding', `${contentsInnerHeightNoPadding}px`);
};
...
export const resizeVerticalSize = (filterHeight: number) => {
  ...
  console.log('themeSet height', height);
...
export const setVerticalSize = (compact: string, height: number, themeData: IThemeData) => {
  ...
  const filterHeight = +getComputedStyle(document.documentElement).getPropertyValue('--size-control-filter-height').replace('px', '');
...
export const setHorizontalSize = (compact: string, width: number, drawerWidth: number, themeData: IThemeData, menuLocation: string) => {
  const sizeData = compact === 'normal' ? themeData.normal : themeData.compact;
  const contentsPadding = sizeData.contentsPadding || 0;
