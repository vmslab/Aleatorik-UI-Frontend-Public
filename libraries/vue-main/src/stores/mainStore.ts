...
export const useMenuStore = defineStore("menu", {
  ...
  actions: {
    ...
    setCurrentMenu(value: IMenu) {
      ...
      this.changeCurrentMenu(value);

      const categoryMenu = menuItems.items.find(t => t.menuId === value.categoryId)!;

      if (!categoryMenu) {
        if (value.categoryId === "Favorite") {
          this.changeRootMenu({ menuId: "Favorite", type: "Category", name: "" });
        } else {
...
export const getNavis = (menu: string, navis: string[]): string[] => {
  const menuItems = useMenuItems();
  const fitem = menuItems.items.find(item => item.id === menu);
  if (fitem) {
    navis.push(fitem.name);
    if (fitem.category) {
      return getNavis(fitem.category!, navis);
    }
  }
  return navis;
};
