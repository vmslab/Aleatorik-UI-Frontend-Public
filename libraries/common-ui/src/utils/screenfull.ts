  ...
  for (const methodList of methodMap) {
    ...
    if (exitFullscreenMethod in document) {
      for (const [index, method] of methodList.entries()) {
        (returnValue as any)[unprefixedMethods[index]] = method;
      }
...
class ScreenFull {
  ...
  request(element: HTMLElement = document.documentElement, options?: IFullScreenOptions): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.options.pageOnly) {
        ...
        const returnPromise = (element as any)[nativeAPI.requestFullscreen](this.options?.fullscreenOptions);
