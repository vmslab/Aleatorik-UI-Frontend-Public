...
export async function ExcelAdvancedTable(param?: any) {
  return await requestFile(
    `/api/${systemID}ExcelAdvancedTable`,
    "post",
    param,
...
export async function OverrideExcelTable(tableName: string, param: any) {
  return await restCall(`/api/${systemID}OverrideExcelTable/${tableName}`, "post", {
    items: JSON.stringify(param),
  });
}
