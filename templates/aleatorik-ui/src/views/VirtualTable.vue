...
import { GetTableRemote, SaveChanges } from "@/api/mainService";
import {
  dateToFormat,
  setOnEditing,
  setCreateProperty,
...
import { EventBus, MenuItem } from "mozart-common";
...
export default class SimpleEditTableWithVersion extends Vue {
  ...
  public async mounted() {
    ...
    if (this.options == null || this.options.params == null) {
      this.options.params = sample.virtual;
    }
  ...
  public setColumnSettings(columnInfos: string[]) {
    return new Promise(async resolve => {
      for await (const info of columnInfos) {
        ...
        const type = infos.length > 1 ? infos[1] : "String";
        this.columnSetting.push({
          key: infos[0],
          name: infos[0],
