<template>
  <div
    ref="botDrawer"
    :class="`dx-card bottom-drawer ${this.className || ''} ${this.value ? 'is-open' : ''}`"
  >
...
export default class BottomDrawer extends Vue {
  ...
  public mounted() {
    ...
    const drawerHeight = getComputedStyle(
      document.getElementsByClassName(this.className || "bottom-drawer")[0],
    ).getPropertyValue("height");
    this.contentsHeight = this.getStringToHeight(drawerHeight) - titleHeight;

    // if (this.parentsHeight) {
    //   if (typeof this.parentsHeight === "string") {
    //     const parentsHeight = getComputedStyle(document.documentElement).getPropertyValue(
    //       this.parentsHeight,
    //     );

    //     this.contentsHeight = this.getStringToHeight(parentsHeight) - titleHeight;
    //   } else typeof this.parentsHeight === "number";
    //   {
    //     this.contentsHeight = +this.parentsHeight - titleHeight;
    //   }
    // }
  }
