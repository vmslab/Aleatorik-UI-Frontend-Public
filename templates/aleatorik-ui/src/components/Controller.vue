  ...
  <div class="moz-controller-root">
    <div class="moz-controller">
      ...
      <div class="moz-controller-title">
        ...
        <slot name="favorite">
          <button class="filter-icon" style="margin-left: 5px">
            <i v-tooltip="{ text: $t('즐겨찾기') }" class="mozart-icons moz-favorite" />
          </button>
          <!-- <i
          id="favorite-btn"
          v-if="!isDev && !isAdmin && !isFavoriteMenu"
          v-tooltip="{ text: $t("즐겨찾기") }"
          @click="onClickFavorite"
          class="mozart-icons moz-favorite top-app-button"
        ></i>
        <i
          id="favorite-btn"
          v-if="!isDev && !isAdmin && isFavoriteMenu"
          ...
          class="mozart-icons moz-favorite-fill top-app-button"
        ></i> -->
        </slot>
...
import { BaseStore } from "mozart-component-wijmo/src/store/modules/baseStore";
// import { MyMenuModule } from "mozart-component-dev/src/store/modules/myMenuStore";
// import { MenuModule } from "mozart-component-dev/src/store/modules/menuStore";
...
export default class Contoller extends Vue {
  ...
  // public async onClickFavorite() {
  //   const menuId = this.$router.currentRoute.meta!.id;
  //   const myMenu = await MyMenuModule.getNewItem();
  //   myMenu.MENU_ID = menuId;
  //   myMenu.SYSTEM_ID = MainModule.getSystemId;
  //   myMenu.SEQUENCE = this.getMyMenuSequence();
  //   const result = await MyMenuModule.addData(myMenu);
  //   this.onResult(result);
  // }

  // public async removeFavorite() {
  //   const menuId = this.$router.currentRoute.meta!.id;
  //   const myMenu = MyMenuModule.getItems.find(t => t.MENU_ID === `F${menuId}`);
  //   if (myMenu) {
  //     const result = await MyMenuModule.removeData(myMenu);
  //     await MyMenuModule.loadData(true);
  //     this.$emit("repaint-menu");
  //     this.onResult(result);
  //   }
  // }

  // public onResult(result: boolean) {
  //   if (result) {
  //     this.onDone();
  //   } else {
  //     this.onError(result);
