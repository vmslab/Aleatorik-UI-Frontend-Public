...
let monitoringSocketURL: string | undefined = process.env!.VUE_APP_WEB_SOCKET;
export const getMonitoringSocketURL = () => {
  if (!monitoringSocketURL) {
    const scheme = window.location.protocol === "https:" ? "wss" : "ws";
    const baseUrl = `${scheme}://${window.location.host}/ws/${systemID}`;
    return baseUrl;
  }
  return monitoringSocketURL ?? "";
};
